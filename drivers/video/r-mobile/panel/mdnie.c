...
int mdnie_send_sequence(struct mdnie_info *mdnie, const struct specific_cmdset *cmdset)
{
 	int ret = 0; //i = 0;
	void *screen_handle;
 ...
 static int ldi_cabc_on(void)
{
   int ret = 0;
	...
	ret = panel_specific_cmdset(screen_handle, cabc_on);
...
void set_mdnie_value(struct mdnie_info *mdnie, u8 force)
{
     ...
     switch (mdnie->scenario) {
	  ...
	  default:
             mdnie_send_sequence(mdnie, bypass_tuning_cmdset);
		break;
     }
...
#ifdef CONFIG_HAS_EARLYSUSPEND
 	mdnie->early_suspend.suspend = NULL;
 	mdnie->early_suspend.resume = NULL;
