...
static int k3dh_do_calibration(void)
{
	...
	g_k3dh->cal_data.x = sum[0] / CALIBRATION_DATA_AMOUNT;
	g_k3dh->cal_data.y = sum[1] / CALIBRATION_DATA_AMOUNT;

        if(sum[2] >= 0) {
#if defined(CONFIG_SENSORS_ACC_16BIT)		
	    g_k3dh->cal_data.z = (sum[2] / CALIBRATION_DATA_AMOUNT) - 16384;    //K2HH(16bit) 16384 +- 3082
#else 
            g_k3dh->cal_data.z = (sum[2] / CALIBRATION_DATA_AMOUNT) - 1024;       //K2DH(12bit) 1024 +-226
#endif
        } else {
#if defined(CONFIG_SENSORS_ACC_16BIT)		
	    g_k3dh->cal_data.z = (sum[2] / CALIBRATION_DATA_AMOUNT) + 16384;    //K2HH(16bit) 16384 +- 3082
#else
            g_k3dh->cal_data.z = (sum[2] / CALIBRATION_DATA_AMOUNT) + 1024;       //K3DH(12bit) 1024 +-226
#endif
        }
...
static irqreturn_t k3dh_accel_interrupt_thread(int irq, void *k3dh_data_p)
{
	...
	wake_lock_timeout(&g_k3dh->reactive_wake_lock, msecs_to_jiffies(3000));
...
static int k3dh_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_abs_params(input_dev, REL_X,-32768, 32767, 0, 0);
	/* acceleration y-axis */
	input_set_capability(input_dev, EV_REL, REL_Y);
	input_set_abs_params(input_dev, REL_Y,-32768, 32767, 0, 0);
	/* acceleration z-axis */
	input_set_capability(input_dev, EV_REL, REL_Z);
	input_set_abs_params(input_dev, REL_Z,-32768, 32767, 0, 0);
...
static int __init k3dh_init(void)
{
	...
	if (device_create_file(dev_t, &dev_attr_accel_cal_open) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n", dev_attr_accel_cal_open.attr.name);
